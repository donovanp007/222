...
export function usePatients() {
  ...
  const searchPatients = (searchTerm: string) => {
    ...
    return patients.filter(patient => {
      ...
      // Basic patient info search
      const basicMatch = fullName.includes(searchLower) ||
      ...
      // Deep content search in sessions
      try {
        ...
        if (savedSessions) {
          ...
          return patientSessions.some((session: any) => {
            // Search in session content
            if (session.content && session.content.toLowerCase().includes(searchLower)) {
              return true;
            }
            
            // Search in template data
            if (session.templateData) {
              const templateContent = Object.values(session.templateData).join(' ').toLowerCase();
              if (templateContent.includes(searchLower)) {
                return true;
              }
            }
            
            // Search in diagnosis
            if (session.diagnosis && Array.isArray(session.diagnosis)) {
              return session.diagnosis.some((diag: string) => diag.toLowerCase().includes(searchLower));
            }
        ...
        console.error('Error searching session content:', error);
      }
